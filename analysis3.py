...
def train_final_model(hyper_arg,which,scale_out):
	hyper_val =  hyper.iloc[hyper_arg]
	pca=False
	create_final_model(hyper_arg,hyper_val,which,pca,scale_out)
	return None
	...
	for i in range(131220):
		if  Path('text_out/Angles_hyper_'+str(i)+'_k_fold_4.txt').is_file():
			...
			if tmp < start:
				...
				if count>10:
					train_final_model(i,which,scale_out)
			None
		else:
			None
...
train_final_model(21281,'JRF',scale_out=True)
